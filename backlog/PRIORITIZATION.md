# Strategic| 1 | ADH-015 | End-to-End Kaggle NFL Scores | adhoc | ba### Immediate Revenue Impact (Priority 1-6)
...
- **Advanced features**: Regime change detection and real-time adjustments for power usersOUNDATION** - We have Kaggle data sitting unused. This unlocks historical context for all models |
| 2 | LLM-014 | LLM Feature Extraction from News | core | accepted | ✅ **COMPLETED** - LLM signal extraction pipeline operational with OpenAI API |
...
| 6 | MOD-003 | Probability calibration & reliability | modeling | backlog | 🎯 **TRUST & ACCURACY** - Ensures prediction probabilities are properly calibrated |og Prioritization
...
2. **LLM-002**: Make predictions explainable and trustworthy *(LLM-014 completed)*
3. **MOD-001**: Establish strong model baselines for reliable predictions

### Month 1 Priority  
4. **MOD-002**: Integrate all signals with intelligent stacking
5. **MOD-003**: Ensure prediction probabilities are properly calibrated
6. **LLM-004**: Add regime change detection for advanced signals

### Key Dependencies
- **ADH-015 → MOD-001**: Historical context enables better model training
- **LLM-014 ✅ → LLM-002**: Feature extraction complete, now add traceability
